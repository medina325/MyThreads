...
head_queue *stack = NULL; // WANNABE stack
...
{
	printf("Preemptado devido ao tempo\n");
 	yield();
}
...
void scheduler()
{
	...
	if(thread_stack.state == indoEnfileirar)
	{

		// eḿpilhando na pilha de suspensos
		enqueue(stack, thread_stack);

		thread_stack.state = waiting;
	...
	else if(thread_aux.state != terminated)
	{
...
int init(void (*main)()){
	...
	// Inicializando contexto da thread stack:
	thread_stack.state = noWaiting;
	...
	// Inicializando stack
	stack = init_queue(stack);
	
	// Inicializa thread atual
...
void done(){
	...
	// Caso a thread_stack.state estiver em waiting, enfileiramos esta thread.
	if( thread_stack.state == waiting )
	{
		printf("Medina Feion\n");
		// Desempilha
		thread_aux = dequeue(stack);

		// Enfileira e muda seu estado para ready
		thread_aux.state = ready;
		enqueue(queue, thread_aux);	
...
tid_t join() {
	...
	thread_stack = *thread_atual;

	thread_stack.state = indoEnfileirar;
	getcontext(&(thread_stack.ctx));


	swapcontext(&(thread_stack.ctx), &scheduler_ctx);


	return thread_stack.tid;
}
