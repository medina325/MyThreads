...
void spawn(void (*start)(), int *tid, int p)
{
	if(p < 0 || p > 10){
		printf("Prioridade fora dos limites. Estabelecida a prioridade padrão\n");
		p = 0;
	}
...
void enqueue(head_queue *queue, thread_t t){
	...
	if(new == NULL) exit(1);
		...
		if(queue->first == NULL ||
			new->t.priority > queue->first->t.priority){
			new->next = queue->first;
			queue->first = new;
		}
		else{
			search = queue->first;
			while(search->next != NULL && new->t.priority <= search->next->t.priority)
				search = search->next;
